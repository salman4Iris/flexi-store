  ...
  return (
    <Suspense
      fallback={
        <div className="max-w-7xl mx-auto px-4 py-10">
          <div className="mb-6">
            <div className="h-8 bg-(--color-text) bg-opacity-10 rounded w-48 mb-4 animate-pulse"></div>
            <div className="h-4 bg-(--color-text) bg-opacity-10 rounded w-96 animate-pulse"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="bg-(--color-text) bg-opacity-10 rounded-lg h-64 animate-pulse"
              ></div>
            ))}
          </div>
        </div>
